...
enum ApiRequesterError: LocalizedError {
    ...
    var localizedDescription: String {
        ...
        case .requestFailed(let stringUrl):
            return "Request failed : \(stringUrl ?? "nil")"
        case .invalidStatusCode(let stringUrl, let statusCode):
...
public final class ApiRequester {
    ...
    // MARK: - Methods
    public func fetch<T: ApiEndpoint>(_ endpoint: T, with parameters: T.RequestDataType) -> Single<
        T.ResponseDataType
    > {
        ...
        return Single.create { [weak self] (single) in
            ...
            let task = self.urlSession.dataTask(with: urlRequest) { (data, response, error) in
                guard
                    ...
                    return single(.error(error ?? ApiRequesterError.unknown))
                }

                guard let urlResponse = response as? HTTPURLResponse else {
                    return single(
                        .error(ApiRequesterError.requestFailed(urlRequest.url?.absoluteString))
                    )
                }
                ...
                guard 200 ..< 300 ~= urlResponse.statusCode else {
                    return single(
                        .error(
                            ApiRequesterError.invalidStatusCode(
